...
public interface UserGraph {
  ...
  public fun provideUserContext(applicationContext: ApplicationContext, userHandle: UserHandle): UserContext {
    val context = try {
      val method = Context::class.java.getMethod("createContextAsUser", UserHandle::class.java, Int::class.javaPrimitiveType)
      method.invoke(applicationContext.context, userHandle, 0) as Context
    } catch (e: Exception) {
